...
```bash
test-vscode$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
test-vscode$ sam local start-api
test-vscode$ curl http://localhost:3000/
```
...
```bash
test-vscode$ sam logs -n HelloWorldFunction --stack-name test-vscode --tail
```
...
```bash
sam delete --stack-name test-vscode
```
